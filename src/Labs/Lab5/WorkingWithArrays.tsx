...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <h3>Updating the Completed Property</h3>
      <div className="d-flex align-items-center mb-3">
        <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
               onChange={(e) => setTodo({...todo, id: e.target.value})}/>
        <input defaultValue={todo.id} className="form-check-input me-2"
               id="wd-todo-completed"
               type="checkbox"
               checked={todo.completed}
               onChange={(e) => setTodo({...todo, completed: e.target.checked})}/>
        <label htmlFor="wd-todo-completed" className="me-auto">
          todo Completed
        </label>
        <a id="wd-update-completed-property" className="btn btn-primary"
           href={`${API}/${todo.id}/completed/${todo.completed}`}>
          Update Todo's Completed with ID = {todo.id} </a>
      </div>
      <hr/>
      ...
      <a id="wd-update-todo-description" className="btn btn-primary float-end"
        ...
        Update Todo's Description with ID = {todo.id} </a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
             onChange={(e) => setTodo({...todo, id: e.target.value})}/>
      <input defaultValue={todo.description} className="form-control w-25"
